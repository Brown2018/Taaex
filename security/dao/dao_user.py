...
class dao_user(object):
    ...
    def getUtilisateur(id):
        try:
            getuser_id=User.objects.get(pk=id)
            utilisateur = Model_Utilisateur.objects.filter(utilisateur=getuser_id.id).select_related('utilisateur')    
    ...
    @staticmethod
    def getUtilisateurByUsername(username):
        try:
            utilisateur = Model_Utilisateur.objects.select_related('utilisateur').get(utilisateur__username=username)                  
            return utilisateur
        except Exception as e:
            print("IL Y A PAS D'UTILISATEUR AVEC CETTE IDENTIFIANT USERNAME  Backend((getUtilisateur)) err=",e)
    
    @staticmethod
    def creationUtilisateur(user,sexe,date,pays,adresse,tel):
        try:
    ...
    def creationVendeur(user,userMaster,sexe,pays,adresse,tel,shop):
        try:
            ...
            utilisateurUser.utilisateur=user
            utilisateurUser.utilisateur_esclave=userMaster
            utilisateurUser.sexe=sexe
            utilisateurUser.pays=pays
            utilisateurUser.adress=adresse
            utilisateurUser.telephone=tel
